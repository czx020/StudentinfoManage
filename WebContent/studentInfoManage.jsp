	...
	<script type="text/javascript">
		...
		function searchStudent() {         //搜索功能
			...
			$('#dg').datagrid('load', {
				...
				bbirthday : $('#s_bbirthday').datebox("getValue"),//Value的V大写！
				ebirthday : $('#s_ebirthday').datebox("getValue"),
				gradeId : $('#s_gradeId').combobox("getValue")
			});
		}
		
		function deleteStudent() {  //删除功能
			var selectedRows = $("#dg").datagrid('getSelections');
			if (selectedRows.length == 0) {
				$.messager.alert("系统提示", "请选择一行需要删除的数据!");
				return;
			}
			var strIds = [];
			...
			$.messager.confirm("系统提示", "您确认要删除这<font color=red>"
				...
				if (r) {
					$.post("studentDelete", {
						...
						if (result.success) {
							$.messager.alert("系统提示", "您已经成功删除<font color=red>"
		...
		function openStudentAddDialog() {   //打开添加对话框
			$("#dlg").dialog("open").dialog("setTitle","添加学生信息");
			url="studentSave";
		}
		
		function saveStudent(){  //保存
			$("#fm").form("submit",{
				url:url,
				onSubmit:function(){
					if($('#sex').combobox("getValue")==""){
						$.messager.alert("系统提示", "请选择性别!");
						return false; //这里返回值是false
					}
					if($('#gradeId').combobox("getValue")==""){
						$.messager.alert("系统提示","请选择所属班级");
						return false;
					}
					return $(this).form("validate"); //必填项
				},
		...
		function openStudentModifyDialog(){   //修改
			var selectedRows=$("#dg").datagrid('getSelections');
		...
		function closeStudentDialog(){  //关闭对话框
			$("#dlg").dialog("close");
			resetValue();
		}
	</script>
	...
	<body>
		...
		<div id="tb">
				...
				<div>
						...
						&nbsp;出生日期:&nbsp;<input class="easyui-datebox" name="s_bbirthday" id="s_bbirthday"/>-><input class="easyui-datebox" name="s_ebirthday" id="s_ebirthday"/>
						&nbsp;所属班级:&nbsp;<input class="easyui-combobox"  name="s_gradeId" id="s_gradeId" data-options="panelHeight:'auto',editable:false,valueField:'id',textField:'gradeName',url:'gradeComboList'"/>
						<a href="javascript:searchStudent()" class="easyui-linkbutton" iconCls="icon-search" plain="true">搜索</a>
				</div>
		</div>
	
		<div id="dlg" class="easyui-dialog" style="width: 570px;height: 350px;padding: 10px 20px ; z-index:5555"
